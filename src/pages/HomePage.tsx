...
// Fix: Defined props with an interface to avoid potential typing issues.
...
const ToggleButton: React.FC<ToggleButtonProps> = ({ active, onClick, children }) => (
    <button onClick={onClick} className={`w-1/2 py-2 text-sm rounded-md transition-colors duration-200 ${active ? 'bg-brand-cyan text-white' : 'bg-brand-dark-2 text-gray-300 hover:bg-gray-700'}`}>
      {children}
    </button>
  );
